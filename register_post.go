...
func (env Env) RegisterNewUser(c *gin.Context) {
	...
	q := `INSERT INTO users(first_name, last_name, username, email, password_hash) VALUES($1,$2,$3,$4,$5) ON CONFLICT DO NOTHING`
	result, err := env.DB.Exec(q,NewUser.FirstName, NewUser.LastName, NewUser.Username, NewUser.Email, NewUser.Password)
	if err != nil {
		log.Printf("error occurred while inserting new record into users table: %v", err)
